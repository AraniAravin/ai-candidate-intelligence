...
## Day 21 — Multi-Candidate RAG & Context Stuffing
Earlier the rag chat was answering direct questions like why this candidate is suitable for this job which is direct retrieval of the candidate info and augmenting for the LLM to answer.
But now it is a broader retrieval scope change into answering questions like why is candidate x better than candidate y, a comparision and filtering of multiple candidate and  their skills.

Stuffing Context is a real RAG technique, where the entire data needed to answer a question is retrieved and stuffed into the LLM prompt and it has to do the filtering/comparing itself and answer, at our current scale (as a portfolio project) it seems to be ok, but at production level with 10000+ candidate details this isnt a wise decision and therefore at that point the LLM has to consider some retrival filtering tool before answering a question.

Two different question types tested were:
One that could be directly answered without LLM, just by skill matching - Candidate who have both python and typescript? - could be answered by direct filtering logic
